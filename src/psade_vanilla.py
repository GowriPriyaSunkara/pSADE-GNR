...
RESULTS_ROOT = "GMVAE_PGNN_vanilla_july10_Age"
...
def progress_bar(iterable, **kwargs):
    """Use tqdm when installed; otherwise fall back to the plain iterable."""
    if tqdm is None:
        return iterable
...
def train_gmvae(X_train, X_val, y_train, y_val, target_cols, out_dir):
    gmvae_start_time = time.perf_counter()
    gmvae_start_iso = datetime.now().isoformat(timespec="seconds")
    ...
    for epoch in epoch_iter:
        ...
        if tqdm is not None:
            epoch_iter.set_postfix({
                "train_total": f"{train_metrics['total']:.4f}",
                "val_total": f"{val_metrics['total']:.4f}",
                "val_reg": f"{val_metrics['reg']:.4f}",
            })
    ...
    gmvae_elapsed_seconds = time.perf_counter() - gmvae_start_time
    gmvae_time_row = {
        "stage": "GMVAE",
        "dataset": "",
        "fold": "",
        "target": "",
        "csv_path": "",
        "start_time": gmvae_start_iso,
        "end_time": datetime.now().isoformat(timespec="seconds"),
        "elapsed_seconds": gmvae_elapsed_seconds,
        "elapsed_hms": format_seconds(gmvae_elapsed_seconds),
    ...
    print(f"[GMVAE] Training time: {format_seconds(gmvae_elapsed_seconds)}")

    return model, history_df, gmvae_time_row
...
def evaluate_single_target(model, Xv, yv, edge_indices, edge_weights, scaler_y, target_idx: int):
    ...
    with torch.no_grad():
        ...
        Xv_t = torch.as_tensor(Xv, dtype=torch.float32, device=model_device)
        h = model.encode(Xv_t)
...
def train_pgnn_for_target(Z_train, y_train_1d, Z_val, y_val_1d, scaler_y, target_idx, target_name, out_dir):
    pgnn_start_time = time.perf_counter()
    pgnn_start_iso = datetime.now().isoformat(timespec="seconds")
    rows = []
    best_global = None

    total_pgnn_configs = sum(
        len(K_CHOICES_BY_P[p])
        for p in HIGH_P_VALUES
        if p in K_CHOICES_BY_P
    ) * len(KNN_LIST)

    pgnn_pbar = tqdm(
        total=total_pgnn_configs,
        desc=f"PGNN {target_name}",
        dynamic_ncols=True
    ...
    for knn_k in KNN_LIST:
        ...
        # Move graph tensors to the selected device. Graph construction itself
        # remains in scikit-learn on CPU, but all PGNN graph operations run on GPU.
        ...
        if knn_k == KNN_LIST[0]:
            print(f"[PGNN][{target_name}] data device: {data_train.x.device}")
            print(f"[PGNN][{target_name}] graph device: {eis_train[0].device}")
        ...
        for p in HIGH_P_VALUES:
            ...
            for K in K_CHOICES_BY_P[p]:
                ...
                if knn_k == KNN_LIST[0] and p == HIGH_P_VALUES[0] and K == K_CHOICES_BY_P[p][0]:
                    print(f"[PGNN][{target_name}] model device: {next(model.parameters()).device}")
                ...
                if pgnn_pbar is not None:
                    pgnn_pbar.set_postfix({
                        "knn": knn_k,
                        "K": K,
                        "p": p,
                        "RMSE_orig": f"{final['rmse_orig']:.4f}",
                        "R2": f"{final['r2']:.4f}",
                    })
                    pgnn_pbar.update(1)
    ...
    pgnn_elapsed_seconds = time.perf_counter() - pgnn_start_time
    pgnn_time_row = {
        "stage": "PGNN",
        "dataset": "",
        "fold": "",
        "target": target_name,
        "csv_path": "",
        "start_time": pgnn_start_iso,
        "end_time": datetime.now().isoformat(timespec="seconds"),
        "elapsed_seconds": pgnn_elapsed_seconds,
        "elapsed_hms": format_seconds(pgnn_elapsed_seconds),
        "epochs_completed": "",
    ...
    print(f"[PGNN][{target_name}] Training time: {format_seconds(pgnn_elapsed_seconds)}")

    return results_df, pgnn_time_row
...
def run_one_dataset(csv_path, fold_id=None, train_idx=None, val_idx=None):
    ...
    fold_label = f"fold_{fold_id}" if fold_id is not None else "single_split"
    out_dir = os.path.join(RESULTS_ROOT, dataset_name, fold_label) if USE_KFOLD else os.path.join(RESULTS_ROOT, dataset_name)
    os.makedirs(out_dir, exist_ok=True)

    print(f"\n{'=' * 90}")
    print(f"Running dataset: {csv_path}")
    if fold_id is not None:
        print(f"K-fold: {fold_id}/{N_FOLDS}")
    print(f"{'=' * 90}")

    X_train, X_val, Y_train, Y_val, meta = load_dataset_for_gmvae(
        csv_path, train_size=TRAIN_SIZE, seed=SEED, train_idx=train_idx, val_idx=val_idx
    )
    ...
    gmvae_model, history_df, gmvae_time_row = train_gmvae(
        X_train, X_val, Y_train, Y_val, meta["target_cols"], gmvae_dir
    )
    gmvae_time_row.update({
        "dataset": dataset_name,
        "fold": fold_id if fold_id is not None else 0,
        "csv_path": csv_path,
    })
    process_time_rows = [gmvae_time_row]
    ...
    target_iter = progress_bar(
        list(enumerate(meta["target_cols"])),
        desc=f"{dataset_name} targets",
        dynamic_ncols=True
    )

    for target_idx, target_name in target_iter:
        print(f"\n--- Training PGNN for target: {target_name} ---")
        target_dir = os.path.join(pgnn_dir, target_name)
        os.makedirs(target_dir, exist_ok=True)

        res_df, pgnn_time_row = train_pgnn_for_target(
            Z_train=Z_train,
        ...
        pgnn_time_row.update({
            "dataset": dataset_name,
            "fold": fold_id if fold_id is not None else 0,
            "csv_path": csv_path,
        })
        process_time_rows.append(pgnn_time_row)
        ...
        best_row["fold"] = fold_id if fold_id is not None else 0
    ...
    master_df.insert(1, "fold", fold_id if fold_id is not None else 0)
    ...
    dataset_elapsed_seconds = time.perf_counter() - dataset_start_time
    dataset_time_row = {
        "stage": "FULL_DATASET_FOLD" if fold_id is not None else "FULL_DATASET",
        "dataset": dataset_name,
        "fold": fold_id if fold_id is not None else 0,
        "target": "",
        "csv_path": csv_path,
        ...
        "end_time": datetime.now().isoformat(timespec="seconds"),
        "elapsed_seconds": dataset_elapsed_seconds,
        "elapsed_hms": format_seconds(dataset_elapsed_seconds),
        "epochs_completed": "",
        "configs_completed": "",
        "n_datasets": "",
        "n_folds": "",
    ...
    process_time_rows.append(dataset_time_row)

    print("\nTop runs for dataset:")
    print(master_df.sort_values(by=["RelRMSE_orig_pct", "RMSE_orig"]).head(15))
    print(f"[DATASET][{dataset_name}] Total time: {format_seconds(dataset_elapsed_seconds)}")
...
def make_kfold_splits(csv_path, n_splits=N_FOLDS, seed=SEED):
    """Return 5-fold train/validation indices for one dataset."""
    df = pd.read_csv(csv_path)
...
def main():
    ...
    full_start_iso = datetime.now().isoformat(timespec="seconds")
    ...
    if USE_KFOLD:
        ...
        for csv_path in CSV_LIST:
            for fold_num, (train_idx, val_idx) in enumerate(make_kfold_splits(csv_path), start=1):
                run_plan.append((csv_path, fold_num, train_idx, val_idx))

        dataset_iter = progress_bar(
            run_plan,
            desc=f"Datasets x {N_FOLDS}-fold",
            dynamic_ncols=True
        )

        for csv_path, fold_num, train_idx, val_idx in dataset_iter:
    ...
    if len(all_dataset_summaries) > 0:
        ...
        global_df.to_csv(os.path.join(RESULTS_ROOT, "all_datasets_all_folds_summary.csv"), index=False)

        best_df = (
            global_df.sort_values(
                by=["dataset", "fold", "target", "RelRMSE_orig_pct", "RMSE_orig", "R2"],
                ascending=[True, True, True, True, True, False]
            )
            .groupby(["dataset", "fold", "target"], as_index=False)
            .first()
        )
        best_df.to_csv(os.path.join(RESULTS_ROOT, "best_runs_across_all_datasets_all_folds.csv"), index=False)

        cv_mean_df = (
            best_df.groupby(["dataset", "target"], as_index=False)
            .agg(
                folds_completed=("fold", "nunique"),
                mean_RMSE_orig=("RMSE_orig", "mean"),
                std_RMSE_orig=("RMSE_orig", "std"),
                mean_RelRMSE_orig_pct=("RelRMSE_orig_pct", "mean"),
                std_RelRMSE_orig_pct=("RelRMSE_orig_pct", "std"),
                mean_R2=("R2", "mean"),
                std_R2=("R2", "std"),
            )
        )
        cv_mean_df.to_csv(os.path.join(RESULTS_ROOT, "kfold_5_average_metrics.csv"), index=False)
    ...
    if len(all_best_per_target) > 0:
        ...
        combined_best_targets_df.to_csv(
            os.path.join(RESULTS_ROOT, "combined_best_per_target_all_folds_summary.csv"),
            index=False
        )

    full_elapsed_seconds = time.perf_counter() - full_start_time
    full_end_iso = datetime.now().isoformat(timespec="seconds")

    full_run_time_row = {
        "stage": "FULL_RUN_ALL_DATASETS",
        "dataset": "ALL",
        "fold": "ALL",
        "target": "ALL",
        "csv_path": ";".join(CSV_LIST),
        "start_time": full_start_iso,
        "end_time": full_end_iso,
        "elapsed_seconds": full_elapsed_seconds,
        "elapsed_hms": format_seconds(full_elapsed_seconds),
        "epochs_completed": "",
        "configs_completed": "",
        "n_datasets": len(CSV_LIST),
        "n_folds": N_FOLDS if USE_KFOLD else 1,
        "total_dataset_runs": (len(CSV_LIST) * N_FOLDS) if USE_KFOLD else len(CSV_LIST),
        "device": DEVICE,
    }
    all_time_rows.append(full_run_time_row)

    pd.DataFrame(all_time_rows).to_csv(
        os.path.join(RESULTS_ROOT, "full_process_training_times.csv"),
        index=False
    )

    print(f"\nDone. Results saved in: {RESULTS_ROOT}")
    print(f"[FULL RUN] Total time: {format_seconds(full_elapsed_seconds)}")
