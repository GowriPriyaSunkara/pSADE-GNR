...
Runs male, female, and combined CSV files for ALM, BMD, and BFP. Results are
written beneath one output directory. The implementation in src/pgnn_conv.py
is imported directly and is not copied or changed here.

 python3 run_pennington_original_pgnn.py \
  --csvs male.csv female.csv penn_data.csv \
  --targets ALM BMD BFP \
  --output PGNN_SWEEP_RESULTS2 \
  --p-values 2 3 5 10  \
  --knn-list 2 5 10 15 20 25 30 35 40 \
  --folds 5 \
  --mu 0.01 \
  --propagation-steps 4 \
  --epochs 1000 \
...
P_VALUES_DEFAULT = [2.0, 3.0, 5.0, 10.0, 1e2]
...
def load_table(csv_path, target):
    ...
    # All three DXA variables are outcomes, not predictors. Age is retained as
    # an input for ALM/BMD/BFP, matching the supplied Pennington columns.
    excluded = set(NON_FEATURE_COLUMNS + DXA_COLUMNS)
...
def run_one_dataset_target(args, csv_path, target, output, device):
    frame, x_raw, y_raw, feature_names = load_table(csv_path, target)
    name = dataset_name(csv_path)
    task_dir = output / name / target
...
def summarize(fold_frame):
    return (
        fold_frame.groupby(
            ["dataset", "csv_path", "target", "p", "knn"], as_index=False
        )
        .agg(
...
def save_best_rmse(summary, output):
    ...
    best = (
        ordered.groupby(["dataset", "target"], as_index=False, sort=False)
        .head(1)
