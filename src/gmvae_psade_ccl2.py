...
RESULTS_ROOT = "GMVAE_PGNN_July22_cc_Age"
...
# ---------------- GMVAE ----------------
# use different numbers Z_max and c_max, targets weights good 1,1,2,2, "ALM", "BMD", "BFP", 
# "ALM": 1.0,    "BMD": 1.0,    "BFP": 2.0,
N_CLUSTERS = 3
LATENT_DIM = 15
...
# Explicit correlation runs in GMVAE latent space
RUNS = [
...
def save_training_time(path: str, rows):
...
def load_dataset_for_gmvae(csv_path, train_size=TRAIN_SIZE, seed=SEED, split_indices=None):
...
def train_gmvae(X_train, X_val, y_train, y_val, target_cols, out_dir):
    ...
    stage_start_iso = datetime.now().isoformat(timespec="seconds")
    ...
    for epoch in tqdm(range(1, GMVAE_EPOCHS + 1), desc="GMVAE training", leave=False):
    ...
    stage_end_iso = datetime.now().isoformat(timespec="seconds")
    save_training_time(
        os.path.join(out_dir, "gmvae_training_time.csv"),
        [{
            "stage": "GMVAE",
            "start_time": stage_start_iso,
            "end_time": stage_end_iso,
            "elapsed_seconds": stage_end_time - stage_start_time,
            "elapsed_hhmmss": format_elapsed(stage_end_time - stage_start_time),
            "completed": True,
        }]
    )
...
# ============================================================
# GRAPH BUILDERS WITH CORRELATION SUPPORT
# ============================================================
...
def _knn_edge_index_l2_weighted(
    ...
    diff = X[row] - X[col]
    num = (diff ** 2) * c[None, :]
    num = np.sqrt(np.maximum(num.sum(axis=1), 1e-12))
...
def build_edge_graphs_by_names(
    ...
    name_map = {
        "l2": _knn_edge_index_l2_weighted if fw_used is not None else _knn_edge_index_l2_unweighted,
    }
    eis = []
    ews = []
    for nm in names:
...
def train_pgnn_for_target(
    ...
    total_model_runs = len(RUNS) * len(KNN_LIST) * sum(
        len(K_CHOICES_BY_P[p]) for p in HIGH_P_VALUES if p in K_CHOICES_BY_P
    )
    pgnn_pbar = tqdm(total=total_model_runs, desc=f"PGNN {target_name}", leave=False)
    ...
    for run in RUNS:
        ...
        if fw_source is not None:
            if fw_source not in all_target_cols:
                print(f"[WARN] Skipping fw_source={fw_source}; available targets: {all_target_cols}")
                continue

            src_idx = all_target_cols.index(fw_source)
            y_source_train_1d = Y_train_all[:, src_idx]

            fw_used = compute_feature_weights_by_label(
                Z_train,
                y_source_train_1d,
                power=fw_power,
                use_abs=fw_abs,
            ...
            np.save(
                os.path.join(out_dir, f"latent_feature_weights_src-{fw_source}_for-{target_name}_{fw_label}.npy"),
                fw_used
            )

        for knn_k in KNN_LIST:
            eis_train, ews_train = build_edge_graphs_by_names(
                Z_train, knn_k, edge_metrics, fw_used=fw_used
            )
            eis_val, ews_val = build_edge_graphs_by_names(
                Z_val, knn_k, edge_metrics, fw_used=fw_used
            )
            ...
            for p in HIGH_P_VALUES:
                ...
                for K in K_CHOICES_BY_P[p]:
                    ...
                    ckpt_path = os.path.join(
                        out_dir,
                        f"best_model_target-{target_name}_fw-{fw_label}_knn{knn_k}_K{K}_p{p}_mu{model_mu:.3g}.pt"
                    )
                    torch.save(
                        {
                    ...
                    row = {
                        ...
                        "target_idx": target_idx,
                        "fw_source": fw_label,
                        "fw_source_target": fw_source if fw_source is not None else "none",
                        "kNN_graph": knn_k,
                        "K": K,
                        "p": p,
                        "mu_used": model_mu,
                    ...
                    print(
                        f"[PGNN][target={target_name}][fw={fw_label}][src={fw_source}] "
                        f"knn={knn_k} K={K} p={p} "
    ...
    stage_end_iso = datetime.now().isoformat(timespec="seconds")
    save_training_time(
        os.path.join(out_dir, f"pgnn_training_time_{target_name}.csv"),
        [{
            "stage": "PGNN",
            "target": target_name,
            "start_time": stage_start_iso,
...
def run_one_fold(csv_path, dataset_name, fold_idx, train_idx, val_idx):
    fold_start_time = time.time()
    fold_start_iso = datetime.now().isoformat(timespec="seconds")

    out_dir = os.path.join(RESULTS_ROOT, dataset_name, f"fold_{fold_idx}")
    os.makedirs(out_dir, exist_ok=True)

    print(f"\n{'=' * 90}")
    print(f"Dataset: {csv_path} | Fold {fold_idx}/{KFOLD_SPLITS}")
    print(f"{'=' * 90}")
    ...
    for i, target_name in enumerate(meta["target_cols"]):
        latent_train_df[f"{target_name}_scaled"] = Y_train[:, i]
        latent_val_df[f"{target_name}_scaled"] = Y_val[:, i]
    ...
    target_progress = tqdm(
        list(enumerate(meta["target_cols"])),
        desc=f"{dataset_name} fold {fold_idx} targets",
        leave=False,
    )

    for target_idx, target_name in target_progress:
        target_progress.set_postfix_str(target_name)
        print(f"\n--- Fold {fold_idx}: Training PGNN for target: {target_name} ---")
        ...
        res_df = train_pgnn_for_target(
            ...
            Y_train_all=Y_train,
            all_target_cols=meta["target_cols"],
            scaler_y=meta["scaler_y"],
            target_idx=target_idx,
            target_name=target_name,
            out_dir=target_dir,
        )
        res_df.insert(0, "fold", fold_idx)
    ...
    fold_end_iso = datetime.now().isoformat(timespec="seconds")
    save_training_time(
        os.path.join(out_dir, "fold_total_time.csv"),
        [{
            "stage": "complete_fold",
            "dataset": dataset_name,
            "fold": fold_idx,
            "start_time": fold_start_iso,
            "end_time": fold_end_iso,
            "elapsed_seconds": fold_end_time - fold_start_time,
            "elapsed_hhmmss": format_elapsed(fold_end_time - fold_start_time),
            "completed": True,
        }],
    )

    return master_df, best_targets_df


def build_kfold_average_tables(dataset_name, folds_df, dataset_out_dir):
    config_cols = [
        "dataset", "target", "target_idx", "fw_source", "fw_source_target",
        "kNN_graph", "K", "p",
    ]
    metric_cols = [
        "RMSE_scaled", "MAE_scaled", "RelRMSE_scaled_pct",
        "RMSE_orig", "MAE_orig", "RelRMSE_orig_pct", "R2",
        "mu_used", "best_epoch", "best_lr",
    ]

    agg_spec = {col: ["mean", "std"] for col in metric_cols}
    avg_df = folds_df.groupby(config_cols, dropna=False).agg(agg_spec).reset_index()
    avg_df.columns = [
    ...
    fold_counts = (
        folds_df.groupby(config_cols, dropna=False)["fold"]
        .nunique()
        .reset_index(name="folds_completed")
    )
    avg_df = avg_df.merge(fold_counts, on=config_cols, how="left")

    ordered_front = config_cols + ["folds_completed"]
    avg_df = avg_df[ordered_front + [c for c in avg_df.columns if c not in ordered_front]]
    avg_df.to_csv(os.path.join(dataset_out_dir, "kfold_average_all_runs.csv"), index=False)
    ...
    best_avg_df.to_csv(
        os.path.join(dataset_out_dir, "best_average_kfold_per_target.csv"),
        index=False,
    )
...
def run_one_dataset(csv_path):
    ...
    dataset_out_dir = os.path.join(RESULTS_ROOT, dataset_name)
    os.makedirs(dataset_out_dir, exist_ok=True)

    raw_df = pd.read_csv(csv_path)
    n_samples = len(raw_df)
    if n_samples < KFOLD_SPLITS:
        raise ValueError(
            f"Dataset {dataset_name} has {n_samples} rows, fewer than KFOLD_SPLITS={KFOLD_SPLITS}."
        )
    ...
    fold_progress = tqdm(
        enumerate(kfold.split(np.arange(n_samples)), start=1),
        total=KFOLD_SPLITS,
        desc=f"{dataset_name} K-fold",
    )

    for fold_idx, (train_idx, val_idx) in fold_progress:
        fold_progress.set_postfix_str(f"fold {fold_idx}/{KFOLD_SPLITS}")
        fold_df, fold_best_df = run_one_fold(
            csv_path, dataset_name, fold_idx, train_idx, val_idx
        )
        all_fold_summaries.append(fold_df)
        all_fold_best.append(fold_best_df)

    folds_df = pd.concat(all_fold_summaries, axis=0, ignore_index=True)
    folds_df.to_csv(os.path.join(dataset_out_dir, "all_folds_master_summary.csv"), index=False)

    fold_best_df = pd.concat(all_fold_best, axis=0, ignore_index=True)
    fold_best_df.to_csv(os.path.join(dataset_out_dir, "best_run_each_fold.csv"), index=False)

    avg_df, best_avg_df = build_kfold_average_tables(
        dataset_name, folds_df, dataset_out_dir
    )

    dataset_end_time = time.time()
    dataset_end_iso = datetime.now().isoformat(timespec="seconds")
    dataset_time_row = {
        "stage": "complete_dataset_kfold",
        "dataset": dataset_name,
        "kfold_splits": KFOLD_SPLITS,
        "start_time": dataset_start_iso,
        "end_time": dataset_end_iso,
        "elapsed_seconds": dataset_end_time - dataset_start_time,
        "elapsed_hhmmss": format_elapsed(dataset_end_time - dataset_start_time),
        "completed": True,
    }
    save_training_time(
        os.path.join(dataset_out_dir, "dataset_total_time.csv"),
        [dataset_time_row],
    )
    ...
    return {
        ...
        "folds_df": folds_df,
        "kfold_average_df": avg_df,
        "best_average_df": best_avg_df,
        "dataset_time_row": dataset_time_row,
    }
...
def main():
    ...
    print(f"K-fold splits: {KFOLD_SPLITS}")
    ...
    dataset_progress = tqdm(CSV_LIST, desc="Datasets")
    for csv_path in dataset_progress:
        dataset_progress.set_postfix_str(_slugify(csv_path))
        result = run_one_dataset(csv_path)
        all_fold_results.append(result["folds_df"])
        all_kfold_averages.append(result["kfold_average_df"])
        all_best_averages.append(result["best_average_df"])
        dataset_time_rows.append(result["dataset_time_row"])

    if all_fold_results:
        pd.concat(all_fold_results, axis=0, ignore_index=True).to_csv(
            os.path.join(RESULTS_ROOT, "all_datasets_all_folds_summary.csv"),
            index=False,
        )

    if all_kfold_averages:
        pd.concat(all_kfold_averages, axis=0, ignore_index=True).to_csv(
            os.path.join(RESULTS_ROOT, "all_datasets_kfold_average_all_runs.csv"),
            index=False,
        )

    if all_best_averages:
        pd.concat(all_best_averages, axis=0, ignore_index=True).to_csv(
            os.path.join(RESULTS_ROOT, "best_average_kfold_across_all_datasets.csv"),
            index=False,
        )

    if dataset_time_rows:
        pd.DataFrame(dataset_time_rows).to_csv(
            os.path.join(RESULTS_ROOT, "dataset_runtime_summary.csv"),
            index=False,
        )
    ...
    overall_row = {
        ...
        "datasets_requested": len(CSV_LIST),
        "kfold_splits": KFOLD_SPLITS,
        "start_time": total_start_iso,
        "end_time": total_end_iso,
        "elapsed_seconds": total_end_time - total_start_time,
        "elapsed_hours": (total_end_time - total_start_time) / 3600.0,
        "elapsed_hhmmss": format_elapsed(total_end_time - total_start_time),
        "device": DEVICE,
        "completed": True,
    }
    save_training_time(
        os.path.join(RESULTS_ROOT, "overall_total_runtime.csv"),
        [overall_row],
    )

    print(f"\nDone. Results saved in: {RESULTS_ROOT}")
    print(f"Overall elapsed time: {overall_row['elapsed_hhmmss']}")
