...
RESULTS_ROOT = "GMVAE_PGNN_Mar26_cc_age1_bfp1"
...
#BFP-1.5, Age-1.5 not good, Age=2 good,and bfp=1, age=1 not good 
TARGET_WEIGHTS_DICT = {
    "ALM": 1.0,
    "BMD": 1.0,
    "BFP": 2.0,
...
# Correlation runs added in the same spirit as the second code
RUNS = [
    {"edge_metrics": ["l2"], "fw_source": None, "fw_power": 0.0,
     "fw_abs": False, "fw_norm": False, "fw_label": "none"},

    {"edge_metrics": ["l2"], "fw_source": "target", "fw_power": 1.0,
     "fw_abs": True, "fw_norm": True, "fw_label": "corr"},
    {"edge_metrics": ["l2"], "fw_source": "target", "fw_power": 2.0,
     "fw_abs": True, "fw_norm": True, "fw_label": "corr2"},
]
    ...
    for run in RUNS:
        ...
        if fw_source == "target":
            fw_used = compute_feature_weights_by_label(
                Z_train, y_train_1d, power=fw_power, use_abs=fw_abs, normalize=fw_norm
            )
            np.save(os.path.join(out_dir, f"latent_feature_weights_{target_name}_{fw_label}.npy"), fw_used)
        ...
        for knn_k in KNN_LIST:
            ...
            for p in HIGH_P_VALUES:
                ...
                for K in K_CHOICES_BY_P[p]:
                    ...
                    print(
                        f"[PGNN][{target_name}][fw={fw_label}] knn={knn_k} K={K} p={p} "
